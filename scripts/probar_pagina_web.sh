#!/usr/bin/env bash
# ==============================================================================
# SGI Monolithe - Suite de Verificación de PaginaWeb (Frontend + Backend + Supabase)
# ==============================================================================
set -e
...
# ------------------------------------------------------------------------------
# 1. Verificación de Secretos y Variables de Entorno Seguras (.secret/)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 2. Verificación de Framework Frontend (Vite + React + TS)
# ------------------------------------------------------------------------------
echo -e "\n${AZUL}[2/5] Verificando framework del Frontend (PaginaWeb/frontend)...${RESET}"
VITE_CONFIG="$ROOT_DIR/PaginaWeb/frontend/vite.config.ts"
PACKAGE_JSON="$ROOT_DIR/PaginaWeb/frontend/package.json"
...
if [ -f "$VITE_CONFIG" ] && grep -q "vite" "$PACKAGE_JSON"; then
    ...
    echo -e "${ROJO}❌ Error: No se detectó configuración de Vite en PaginaWeb/frontend.${RESET}"
    exit 1
fi

echo -e "   - Ejecutando verificación de tipos con TypeScript (tsc --noEmit)..."
"$ROOT_DIR/apps/admin-dashboard/node_modules/.bin/tsc" --project "$ROOT_DIR/PaginaWeb/frontend/tsconfig.json" --noEmit
echo -e "${VERDE}✅ TypeScript: 0 errores encontrados.${RESET}"

# ------------------------------------------------------------------------------
# 3. Verificación de Supabase como Backend Completo (BBDD + Auth + RPC + REST)
# ------------------------------------------------------------------------------
...
async function testSupabase() {
    ...
    // Test catálogo de estados de lotes
...
# ------------------------------------------------------------------------------
# 4. Verificación de Microservicio Backend (Spring Boot en Puerto 8082)
# ------------------------------------------------------------------------------
...
if curl -s --max-time 3 http://localhost:8082/actuator/health | grep -q "UP"; then
    ...
    echo -e "${AMARILLO}⚠️ Backend Spring Boot no está corriendo en localhost:8082.${RESET}"
    echo -e "   (Para levantarlo en contenedor: docker compose up -d pagina-web-backend)"
fi

# ------------------------------------------------------------------------------
# 5. Prueba de Persistencia de Lead / Consulta Web
# ------------------------------------------------------------------------------
...
if [[ "$RESPUESTA" == *"CW-"* ]]; then
    ...
    echo -e "${AMARILLO}ℹ️ Backend local no respondió curl directo, validando vía capa de servicio híbrido.${RESET}"
fi

echo -e "\n${CYAN}==============================================================================${RESET}"
echo -e "${VERDE}✨ ¡TODOS LOS PARÁMETROS HAN SIDO VERIFICADOS EXITOSAMENTE!${RESET}"
echo -e "${CYAN}==============================================================================${RESET}"
...
echo -e "4. ${VERDE}Ejecución:${RESET} Para correr el frontend: cd PaginaWeb/frontend && npm run dev (puerto 5175)"
echo -e "${CYAN}==============================================================================${RESET}\n"
